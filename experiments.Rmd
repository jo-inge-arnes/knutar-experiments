...
plot_curves_and_observations <- function(d, f_curve, mod0, mod1, curve_name) {
  ...
  fig <- fig + theme_bw()
  fig <- fig + geom_point(data = d, aes(Independent, Dependent), shape = 1, color = "gray60")  
  ...
  fig <- fig + scale_color_manual(values = c("gray70", "black", "black"),
                     labels = parse_format()(c(true_label, a0_label, a1_label)))
  fig <- fig + scale_size_manual(values=c(1.7, 1, 1),
                     labels = parse_format()(c(true_label, a0_label, a1_label)))
...
f_signal <- function(x, curve_name) {
  y <- switch(curve_name,
         ...
         "Runge" = 1 / (1 + (x - 2)^2),
         "Quintic" = x^5 - 10*x^4 + 35*x^3 - 50*x^2 + 24*x + 5)
    return(y)
}
