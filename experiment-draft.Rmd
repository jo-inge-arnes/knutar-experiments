...
'
  ...
  res_quant <- choose_splines(d, Dependent, Independent, max_nknots = max_knots, icr_fn = stats::AIC) 
...
```{r}
df_means <- df_all_results %>% group_by(generator_index, generator_sample_index) %>% summarise_at(vars(mse_def, mse_quant, score_def, score_quant), list(mean, sd))
```
