...
if (shapiro.test(df_1$score_def)$p.value >= 0.05 && shapiro.test(df_1$score_quant)$p.value >= 0.05) {
  t_res <- t.test(df_1$score_def, df_1$score_quant, paired = TRUE, alternative = "two.sided")
  mean_ci <- list(test.type = "T-test", conf.int = t_res$conf.int, p.value = t_res$p.value)
} else {
  b1 <- boot(df_1$score_def - df_1$score_quant, function(u,i) mean(u[i]), R = 1000)
...
```{r}
library(scales)
df_1$bic_diff_cats <- cut(df_1$score_def - df_1$score_quant, c(-Inf, -0.5, 0.5, Inf), c("Worse (W)", "Equally good (E)", "Better (B)"), right = FALSE) 
...
ggplot(df_1, aes(x=bic_diff_cats)) + 
  theme_bw() +
  geom_bar() +
...
```{r, echo = FALSE}
pearson_r <- cor(df_1$score_def - df_1$score_quant, df_1$std_diff)
ggplot(df_1, aes(x = score_def - score_quant, y = std_diff)) + 
  theme_bw() +
  geom_point(shape = 1, color = "gray40") +
  stat_smooth(method = "lm", color = "black", size = 0.5) +
  ggtitle(paste("Relationship Between BIC and MSE Differences (r = ", round(pearson_r, 2), ")")) +
  ylab(parse_format()("MSE[A[0]] - MSE[A[1]]")) + 
  xlab(parse_format()("BIC[A[0]] - BIC[A[1]]"))
  

pearson_r <- cor(df_1$score_def - df_1$score_quant, df_1$std_diff_curve)
ggplot(df_1, aes(x = score_def - score_quant, y = std_diff_curve)) + 
  theme_bw() +
  geom_point(shape = 1, color = "gray40") +
  stat_smooth(method = "lm", color = "black", size = 0.5) +
  ggtitle(paste("Relationship Between BIC and MSE Differences (r = ", round(pearson_r, 2), ")")) +
